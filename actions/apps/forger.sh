...
forger_restart ()
{
    ...
    pm2 restart $commander_ecosystem --only qredit-core-forger >> "$commander_log" 2>&1
...
forger_stop ()
{
    ...
    pm2 stop $commander_ecosystem --only qredit-core-forger >> "$commander_log" 2>&1
...
forger_delete ()
{
    ...
    local delete_forger=$(pm2status "qredit-core-forger" | awk '{print $2}')
    if [[ -n $delete_forger ]]; then
        heading "Deleting Forger..."

        pm2 delete $commander_ecosystem --only qredit-core-forger >> "$commander_log" 2>&1
...
forger_logs ()
{
    ...
    pm2 logs qredit-core-forger
}

forger_status ()
{
    local status=$(pm2status "qredit-core-forger" | awk '{print $13}')
...
__forger_start_with_bip38 ()
{
    ...
    pm2 start $commander_ecosystem --only qredit-core-forger -- --password "$password" >> "$commander_log" 2>&1
}

__forger_start_without_bip38 ()
{
    pm2 start $commander_ecosystem --only qredit-core-forger >> "$commander_log" 2>&1
}
