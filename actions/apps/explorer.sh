...
explorer_install ()
{
    ...
    heading "Installing Qredit Explorer..."
    ...
    success "Installed Qredit Explorer!"
}
...
explorer_uninstall ()
{
    ...
    heading "Uninstalling Qredit Explorer..."

    sudo rm -rf "$EXPLORER_DIR"

    success "Uninstalled Qredit Explorer!"
}
...
explorer_update ()
{
    ...
    if [[ "$remote_version" == "$local_version" ]]; then
        ...
        info "You already have the latest supported Qredit Explorer version."
    else
        STATUS_EXPLORER_UPDATE="Yes"

        read -p "An update is available for Qredit Explorer, do you want to install it? [Y/n] : " choice
...
explorer_start ()
{
    ...
    pm2 start $commander_ecosystem --only qredit-explorer >> "$commander_log" 2>&1
...
explorer_restart ()
{
    ...
    pm2 restart $commander_ecosystem --only qredit-explorer >> "$commander_log" 2>&1
...
explorer_stop ()
{
    ...
    pm2 stop $commander_ecosystem --only qredit-explorer >> "$commander_log" 2>&1
...
explorer_logs ()
{
    ...
    pm2 logs qredit-explorer
}

explorer_status ()
{
    local status=$(pm2status "qredit-explorer" | awk '{print $13}')
