...
database_drop_user ()
{
    ...
    sudo -u postgres dropuser --if-exists "$QREDIT_DB_USERNAME" | tee -a "$commander_log"
...
database_destroy ()
{
    ...
    sudo -u postgres dropdb --if-exists "$QREDIT_DB_DATABASE" | tee -a "$commander_log"
...
database_create ()
{
    ...
    local databaseExists=$(psql -l | grep "${QREDIT_DB_DATABASE}" | wc -l)

    if [[ $databaseExists == 1 ]]; then
        read -p "The database ${QREDIT_DB_DATABASE} already exists, do you want to overwrite it? [y/N] : " choice

        if [[ "$choice" =~ ^(yes|y|Y) ]]; then
            dropdb "${QREDIT_DB_DATABASE}" | tee -a "$commander_log"
            createdb "${QREDIT_DB_DATABASE}" | tee -a "$commander_log"
        fi
    else
        createdb "${QREDIT_DB_DATABASE}" | tee -a "$commander_log"
    fi
